...
ユーザーがプロンプトを送信
  ...
  └─ 返ってきた確率を src/assess.ts で判定する
       ...
       └─ has_*        … 項目ごとの有無（Noul）。0.5 未満を「不足」とする
            │
            ├─ ctx.ui.setStatus() で footer に常設表示（ユーザーが読む）
            │
            └─ before_agent_start で display: false のメッセージを渡す（モデルだけが読む）
```

`input` は拡張コマンド判定の後・skill/template 展開の前に発火する。評価はここで逐次走り、**プロンプト本文は一切書き換えない**。mid-stream の割り込み（`steer`）だけは評価しない。jev の 250〜650ms が修正の即時性を損なうためで、`skipSteering` を `false` にすれば評価対象にできる。
...
- **短い入力も評価する。** 文字数では足切りしない。日本語は数文字で依頼が成立するので（「いい感じにして」は7文字）、文字数を基準にすると本物の依頼を静かに落とす。雑談は `chitchat` 分類が弾くため status は `不足なし` になるが、jev の往復（250〜650ms）は毎回発生する。短い相づちの遅延が気になるなら、`input` ハンドラに下限を足すのが唯一の調整点。
...
- **モデルへの注入は `before_agent_start`。** 評価自体は `input` で逐次走るが、モデルへの受け渡しだけはターン確定後に発火するこのフックを使う。`input` の `transform` はプロンプト本文を書き換えてしまうため採らない。
