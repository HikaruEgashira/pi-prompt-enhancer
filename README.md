...
ユーザーがプロンプトを送信
  │
  ├─ input イベントで状態 { request: <プロンプト> } と 15 個の質問を
  ...
  └─ 返ってきた確率を src/assess.ts で判定する
       ...
       └─ has_*        … 項目ごとの有無（Noul）。0.5 未満を「不足」とする
            │
            └─ 不足があればプロンプト末尾に <prompt_assessment> を追記（transform）
```

`input` は拡張コマンド判定の後・skill/template 展開の前に発火する。`transform` で追記するので、評価はユーザーの発言そのものとして文字起こしに残り、モデルも依頼の一部として読む。mid-stream の割り込み（`steer`）だけは評価しない。jev の 250〜650ms が修正の即時性を損なうためで、`skipSteering` を `false` にすれば評価対象にできる。
...
```
このリポジトリのコードをいい感じに直してほしい。全体的に品質を上げたい。
...
- **短い入力も評価する。** 文字数では足切りしない。日本語は数文字で依頼が成立するので（「いい感じにして」は7文字）、文字数を基準にすると本物の依頼を静かに落とす。雑談は `chitchat` 分類が弾くため出力は出ないが、jev の往復（250〜650ms）は毎回発生する。短い相づちの遅延が気になるなら、`input` ハンドラに下限を足すのが唯一の調整点。
- **直近の 1 メッセージしか見ていない。** 会話の文脈は判定に渡していないため、前のターンで共有済みの項目を「不足」と言うことがある。追記ブロックにその旨を書いてあるので、モデル側は無視できる。会話も state に渡せば解消できる。
